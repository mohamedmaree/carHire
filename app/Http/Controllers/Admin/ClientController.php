...
class ClientController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->middleware('permission:update-' . $name)->only([ 'edit', 'update','updateBalance' ]);
        $this->middleware('permission:delete-' . $name)->only([ 'destroy' ]);
        $this->middleware('permission:notify-' . $name)->only([ 'notify' ]);
    }
    ...
    public function store(Store $request)
    {
        ...
        User::create($request->validated() + [ 'is_blocked' => $is_blocked ,'active' => $active]);
    ...
    public function update(Update $request, $id)
    {
        $is_blocked = ($request->is_blocked) ? 1 : 0;
        $active = ($request->active) ? 1 : 0;
        $user = User::findOrFail($id)->update($request->validated() + [ 'is_blocked' => $is_blocked ,'active' => $active]);
