        ...
        @foreach($partensers as $partenser)
            <tr class="delete_partenser">
                <td class="text-center">
                    <label class="container-checkbox">
                        <input type="checkbox" class="checkSingle" id="{{$partenser->id}}">
                        <span class="checkmark"></span>
                    </label>
                </td>
                <td><img src="{{$partenser->image}}" width="30px" alt=""></td>
                <td class="product-action">
                    @can('read-intro-partener')
                        <span class="text-primary"><a
                                    href="{{ route('admin.introparteners.show', ['intropartener' => $partenser->id]) }}"
                                    class="btn btn-warning btn-sm"><i class="feather icon-eye"></i> {{ __('admin.show') }}</a></span>
                    @endcan
                    @can('update-intro-partener')
                        <span class="action-edit text-primary"><a
                                    href="{{ route('admin.introparteners.edit', ['intropartener' => $partenser->id]) }}"
                                    class="btn btn-primary btn-sm"><i class="feather icon-edit"></i>{{ __('admin.edit') }}</a></span>
                    @endcan
                    @can('delete-intro-partener')
                        <span class="delete-row btn btn-danger btn-sm"
                              data-url="{{ url('admin/introparteners/' . $partenser->id) }}"><i
