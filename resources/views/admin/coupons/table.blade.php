            ...
            <tr class="delete_coupon">
                ...
                <td>
                    @if($coupon->status == 'available')
                        <span class="btn btn-sm round btn-outline-danger change-coupon-status"
                              data-date="{{$coupon->expire_date}}" data-status="closed" data-id="{{$coupon->id}}">
                                {{__('admin.Stop_Coupon')}}  <i class="feather icon-slash"></i>
                            </span>
                    @else
                        <span class="btn btn-sm round btn-outline-success open-coupon" data-toggle="modal"
                              id="div_{{$coupon->id}}" data-target="#notify" data-id="{{$coupon->id}}">
                ...
                <td class="product-action">
                    ...
                    @can('delete-coupon')
                        <span class="delete-row btn btn-danger btn-sm"
                              data-url="{{ url('admin/coupons/' . $coupon->id) }}"><i class="feather icon-trash"></i>{{ __('admin.delete') }}</span>
                    @endcan
                </td>
            </tr>
        @endforeach
