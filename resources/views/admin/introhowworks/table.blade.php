    ...
    <table class="table " id="tab">
        ...
        @foreach($howWorks as $howWork)
            <tr class="delete_howWork">
                ...
                <td><img src="{{$howWork->image}}" width="30px" alt=""></td>
                <td>{{$howWork->title}}</td>
                <td class="product-action">
                    @can('read-intro-how-work')
                        <span class="text-primary"><a
                                    href="{{ route('admin.introhowworks.show', ['introhowwork' => $howWork->id]) }}"
                                    class="btn btn-warning btn-sm"><i class="feather icon-eye"></i> {{ __('admin.show') }}</a></span>
                    @endcan
                    @can('update-intro-how-work')
                        <span class="action-edit text-primary"><a
                                    href="{{ route('admin.introhowworks.edit', ['introhowwork' => $howWork->id]) }}"
                                    class="btn btn-primary btn-sm"><i class="feather icon-edit"></i>{{ __('admin.edit') }}</a></span>
                    @endcan
                    @can('delete-intro-how-work')
                        <span class="delete-row btn btn-danger btn-sm"
                              data-url="{{ url('admin/introhowworks/' . $howWork->id) }}"><i
