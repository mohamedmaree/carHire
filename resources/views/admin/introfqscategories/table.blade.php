        ...
        @foreach($categories as $category)
            <tr class="delete_category">
                ...
                <td class="product-action">
                    @can('read-intro-fqs-category')
                        <span class="text-primary"><a
                                    href="{{ route('admin.introfqscategories.show', ['introfqscategory' => $category->id]) }}"
                                    class="btn btn-warning btn-sm"><i class="feather icon-eye"></i> {{ __('admin.show') }}</a></span>
                    @endcan
                    @can('update-intro-fqs-category')
                        <span class="action-edit text-primary"><a
                                    href="{{ route('admin.introfqscategories.edit', ['introfqscategory' => $category->id]) }}"
                                    class="btn btn-primary btn-sm"><i class="feather icon-edit"></i>{{ __('admin.edit') }}</a></span>
                    @endcan
                    @can('delete-intro-fqs-category')
                        <span class="delete-row btn btn-danger btn-sm"
                              data-url="{{ url('admin/introfqscategories/' . $category->id) }}"><i
