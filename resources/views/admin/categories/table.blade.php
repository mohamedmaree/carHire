    ...
    <table class="table " id="tab">
        ...
        @foreach($categories as $category)
            <tr class="delete_row">
                ...
                <td><img src="{{$category->image}}" width="30px" alt=""></td>
                <td>{{$category->name}}</td>
                <td><a href="{{route('admin.categories.index' , ['id' => $category->id])}}">{{__('admin.view')}}</a>
                </td>
                <td>
                ...
                <td class="product-action">
                    ...
                    @can('delete-category')
                        <span class="delete-row btn btn-danger btn-sm"
                              data-url="{{ url('admin/categories/' . $category->id) }}"><i
