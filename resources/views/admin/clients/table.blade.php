        ...
        @foreach ($rows as $row)
            <tr class="delete_row">
                ...
                <td class="product-action">
                    @can('read-user')
                        <span class="text-primary"><a href="{{ route('admin.clients.show', ['client' => $row->id]) }}"
                                                      class="btn btn-warning btn-sm"><i class="feather icon-eye"></i> {{ __('admin.show') }}</a></span>
                    @endcan
                    @can('update-user')
                        <span class="action-edit text-primary"><a
                                    href="{{ route('admin.clients.edit', ['client' => $row->id]) }}"
                                    class="btn btn-primary btn-sm"><i class="feather icon-edit"></i>{{ __('admin.edit') }}</a></span>
                    @endcan
                    @can('notify-user')
                        <span data-toggle="modal" data-target="#notify" class="notify btn btn-info btn-sm"
                              data-id="{{ $row->id }}" data-url="{{ url('admins/clients/notify') }}"><i
                                    class="feather icon-bell"></i>{{ __('admin.notify') }}</span>
                    @endcan
                    @can('delete-user')
                        <span class="delete-row btn btn-danger btn-sm"
                              data-url="{{ url('admin/clients/' . $row->id) }}"><i class="feather icon-trash"></i>{{ __('admin.delete') }}</span>
                    @endcan
