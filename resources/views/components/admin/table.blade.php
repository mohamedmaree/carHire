<div class="content-body">
    ...
    <div class="card card-body">
        <div class="card-title">{{ __('admin.search_filter') }}</div>
                ...
                <div class="col-md-3 mt-1 col-12">
                    ...
                    <div class="controls ">
                        ...
                        @if ($value['input_type'] == 'text')
                            <input type="text" name="{{$key}}" class="form-control search-input" placeholder="{{$value['input_name']}}"  value="{{ request()->input('searchArray.' . $key) }}">
                        @elseif ($value['input_type'] == 'date')
                            <input type="date" name="{{$key}}" class="form-control search-input" placeholder="{{$value['input_name']}}"  value="{{ request()->input('searchArray.' . $key) }}">
                        @elseif ($value['input_type'] == 'time')
                            <input type="time" name="{{$key}}" class="form-control search-input" placeholder="{{$value['input_name']}}"  value="{{ request()->input('searchArray.' . $key) }}">
                        @elseif ($value['input_type'] == 'number')
                            <input type="number" name="{{$key}}" class="form-control search-input" placeholder="{{$value['input_name']}}"  value="{{ request()->input('searchArray.' . $key) }}">
                        @elseif ($value['input_type'] == 'select')
                        <select name="{{$key}}" class="form-control select2 search-input">
                            <option value="">{{ __('admin.choose') }}</option>
                            @foreach ($value['rows'] as $row)
                                <option value="{{$row['id']}}" 
                                        {{ request()->input('searchArray.' . $key) === $row['id'] || (request($key) == $row['id'] && request($key) != '' && request($key) != null) ? 'selected' : '' }}>
